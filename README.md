...
Проверкам на поднятом стенде также нужны `curl`, `jq`, `awk`, `grep`, `sed`,
`tail`, `sleep` и `timeout`. `jq` нужен и самому `make up`: им читается паспорт
мира, по которому он ждёт заливки. По умолчанию должны быть свободны порты
`23000`, `28080`, `28088`, `28123`, `28124`, `29000`, `29001`, `29090` и
`29092`. Проверкам без стенда — `make config-test` и `make lint` в корне, всем
целям генератора в `generator/` — нужен `uv`.
...
Остановить контейнеры без удаления данных можно командой `make down`. Если нужно
вернуть прикладной мир к началу, `make rebuild-storage` запустит в Airflow его
явную пересборку: удалит прикладные базы ClickHouse, топики и позиции
потребителей Kafka, затем создаст схему и данные заново. Службы и их тома
останутся на месте. `make clean` удаляет весь стенд вместе с именованными томами
и для обычной разработки не нужен. Перед пересборкой поставьте `world_live` на
паузу и дайте уже начатому шаговому дагу закончить день.

## Стартовый мир

Стартовый мир и его паспорт описаны в [справочнике
мира](docs/architecture/world.md).
...
Даги времени, позиция мира и форма запуска описаны в [справочнике
