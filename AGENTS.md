...
Стенд рассчитан на начинающих инженеров данных. Они недавно изучили ClickHouse,
но почти не работали с ним руками. Здесь они разбирают и достраивают хранилище
небольшого интернет-магазина: проходят путь от загрузки данных до витрин и BI.
Программа дает практику общепринятых инженерных приемов. После нее участники
должны уметь построить похожее хранилище без готового образца. Состав и порядок
работы задает [программа учебных заданий](docs/specs/2026-09-01-labs-program.md).

Стенд существует только ради обучения. Его поддержка имеет смысл, когда
сохраняет возможность пройти программу или сама становится учебной задачей.
Отдельной эксплуатационной ценности у стенда нет.

Отсюда вопрос к любой доработке — коду, проверке, документу: **чему она
учит?** Ответ называется одной фразой до того, как писать. Не
...
стенд, тем хуже он как учебный материал: внимание читателя конечно, и каждая
конструкция, которую он обязан расшифровать по дороге к уроку, списывается с
этого счёта. Поэтому вопрос выше имеет вторую половину: **что читатель платит,
...
Автор задачи сам пишет и редактирует код, комментарии и документы, если
владелец не задал другое распределение работы.

Делить работу стоит не целыми задачами, а подзадачами, и решать
...
При создании или редактировании лабораторных следуй
[правилам учебного текста](docs/specs/2026-09-01-labs-program.md#как-писать-задания).
...
- `make clean` полностью удаляет постоянное состояние стенда. Перед этой
  командой назови состояние вне прикладного мира, которое требуется удалить,
  и объясни, почему `make rebuild-storage` его не удаляет. При изменении DDL
  или проверке на чистых данных по умолчанию используй адресную очистку либо
...
- Имена файлов в `docs/formats/` — слаг строчными латинскими буквами через
  ...
  пишется руками.
- Имена файлов основной программы в `docs/labs/` — `S-N-slug.md`:
  номер ступени, номер задания внутри ступени и слаг строчными латинскими
  буквами через дефис (`1-2-top-pages-daily.md`). Здесь живут ТЗ
  учебных заданий. Номера повторяют порядок программы из `README.md`, чтобы
  папка читалась как оглавление; даты нет: задание — живой документ и
  правится вместе с тем, о чем говорит. Слаг называет предмет задания.
  Факультативы живут в `docs/labs/optional/`, без номеров, под одним слагом
  (`superset-dashboard.md`). Общий индекс обеих частей — `docs/labs/README.md`;
  отдельного индекса в `optional/` нет.
- Имена файлов в `docs/handbook/` — слаг строчными латинскими буквами через
  дефис, индекс папки — `README.md`. Здесь живет путеводитель по стенду:
  учебные главы для читателя. Ни дат, ни номеров: глава — живой документ и
  правится вместе с тем, что описывает. Исходники интерактивной карты лежат
  подпапкой `architecture-map/`, а ее рендер — файлом `architecture-map.html`
  рядом с главами.
