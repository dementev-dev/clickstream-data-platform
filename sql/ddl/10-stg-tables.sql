...
--
-- Пояс у обеих меток написан в типе — DateTime64(3, 'UTC'); в DDL стенда он
-- встречается здесь впервые. Само число от пояса не зависит, это секунды от
-- начала эпохи. Пояс — линза: по нему решают, какие часы покажут метку и в
-- какие сутки она попадёт, то есть чем окажется toDate(_load_ts) в ключе
