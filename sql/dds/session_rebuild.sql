-- DDS: сборка сессий выбранных дней в донор dds.session_stage_dist.
-- Донор очищается целиком; одна вставка собирает весь диапазон.
--
...
-- эти единицы: получается номер визита внутри посетителя и дня.
-- PARTITION BY (client_id, event_date) разделяет визиты по модельной полночи.
...
-- признак начала и накопленный номер визита относились к одним событиям.
