"""Трансформации DDS: заказы, сессии и карта идентичностей.

Запросы рассчитывают заказы и сессии за выбранные дни и записывают результат
в промежуточные таблицы. Затем готовые дневные разделы этих таблиц заменяют
разделы основных таблиц. Пока идет расчет, читатели видят прежние данные.

Карта идентичностей пересчитывается целиком после заказов. Новые строки
добавляются с меткой времени сборки; представление с FINAL выбирает последнюю
версию каждой строки.

Даг запускается без расписания и параметров. Даты он выбирает по состоянию
таблиц и позиции мира. Новый запуск заново определяет диапазон: наличие даты
не подтверждает полноту дня и не гарантирует повтор его сборки после сбоя.
Подробности — docs/architecture/etl.md, «Объём прогона» и «Идемпотентность».
"""
...
def replacements(scope: dict[str, object]) -> list[dict[str, object]]:
    """Подготовить параметры отдельной задачи replace для каждого дня.

    expand_kwargs получает список словарей и создает по одному экземпляру
    задачи на каждый словарь. Дата подставляется в SQL шаблоном Jinja:
    параметр ClickHouse в ALTER ... PARTITION здесь не поддерживается.
    """
...
@dag(
    ...
    # Одновременные запуски используют одни промежуточные таблицы.
    # Второй мог бы очистить результат первого до замены партиций.
    max_active_runs=1,
...
def dds_transform():
    ...
    def order():
        """Заказ: изменяемые дни последнего слепка и пропущенные дни."""
        ...
        def scope() -> dict[str, object]:
            ...
            # Без заказов ODS расчет невозможен: задача сообщает об ошибке.
            if row is None:
                raise AirflowException(
        ...
        # На каждый день создается отдельная задача замены партиции.
        # Неудачную замену можно повторить, пока промежуточная таблица
        # сохраняет рассчитанные строки этого запуска.
    ...
    def session():
        ...
        def scope() -> dict[str, object]:
            ...
            first_day, last_day = row
            # Для каждого прожитого дня уже есть строки сессий.
            # Новых дней для расчета нет: группа задач будет пропущена.
