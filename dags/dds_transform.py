...
Идемпотентность здесь устроена не так, как в приёме заказов, и в этом весь
урок сущности. Приём гасит повтор колонкой версии: ReplacingMergeTree
оставляет позднюю строку. Трансформация версий не знает — она собирает день
заново в таблицу-двойник и атомарно подменяет им партицию цели. Поэтому
повтор прогона не меняет делового содержимого слоя, а строки пересобранных
дней получают новый _load_id: видно, каким запуском собран каждый день.

Замена партиции идёт по локальным таблицам с ON CLUSTER: по Distributed
операции с партициями отвечают кодом 48 (docs/architecture/storage.md, «Что
проверено»). Форма двойника и цена ошибки в ключе шардирования разобраны в
sql/ddl/45-dds-tables.sql.
"""
...
# Окно изменяемости заказа, модельные дни. Число мира, а не настройка стенда:
# владелец — generator/world.py (ORDER_WINDOW_DAYS), который этим окном
# собирает слепок. Здесь оно читается, чтобы хранилище знало, какие дни ещё
# могут измениться. Разойдутся два объявления — пересборка промахнётся по краю
# окна, поэтому оба названы в compose.yaml рядом и с этой оговоркой.
