"""Даги модельного времени: ими двигают ось мира.

Их три, и они двух родов. **Шаговые даги** играют день и запускаются руками:
...
не делает: он тикает по расписанию и дёргает шаговый даг живого дня, дожидаясь
...
дождаться, пока хранилище его заберёт. Поэтому у обоих шаговых дагов за проигрышем
...
Разделение не косметическое. Расписание на самом шаговом даге заставило бы кнопку
паузы значить две вещи разом — «мир не едет сам» и «даг выключен», — а шаговый даг
при этом выглядел бы в списке выключенным, хотя нажимают его каждый день.

Календарь Airflow к оси мира отношения не имеет: какой день играть, шаговый даг
...
# шаговые даги не имеют права угадывать её — состояние объяснено в ADR 0013.
#
...
# Какой день играть, шаговый даг узнаёт у первой задачи. Спрашивают её все
...
def _ingest_orders() -> TriggerDagRunOperator:
    ...
    Ожидание здесь несущее. Без него шаговый даг позеленел бы, не узнав, доехал
    ...
    return TriggerDagRunOperator(
        ...
        # Умолчание — минута опроса, а весь прогон шагового дага пачкой короче.
        poke_interval=10,
    )
...
@dag(
    ...
    params={
        "days": Param(
            ...
            minimum=1,
            maximum=7,
            title="Число модельных дней для генерации",
            # description_md рендерится подсказкой прямо в форме запуска
            # (params.rst, Airflow 3.3), чтобы правило было видно сразу.
            description_md=(
                "Создать данные за следующие 1–7 модельных дней; по умолчанию "
                "за один день. События отправляются без пауз. Номер первого дня "
                "берется из `world_position` (Admin → Variables). Поля Logical "
                "date и Run ID не меняют выбор дня. После завершения запустите "
                "`etl_pipeline`, чтобы обновить DDS и витрины DM."
            ),
        )
    },
)
def world_next_day():
    """Создать данные за следующие модельные дни без пауз между событиями.

    Параметр `days` задает число дней: от 1 до 7, по умолчанию 1. Первый день
    берется из `world_position`. После загрузки событий и слепков заказов
    запустите `etl_pipeline` вручную, чтобы обновить DDS и витрины DM.
    """
    ...
    def remember_played(first_day: int) -> None:
        ...
        logging.info(
            "данные за модельные дни %s–%s отправлены; world_position=%s",
            first_day,
            position - 1,
            position,
        )
...
def world_live_day():
    """Постепенно отправить события за следующий модельный день.

    При ускорении ×60 отправка занимает около 24 реальных минут. Номер дня
    берется из `world_position`; Logical date его не меняет. Затем даг
    отправляет слепок заказов и ждет его загрузки и завершения `etl_pipeline`.
    Для автоматического запуска следующих дней снимите с паузы `world_live`.
    """
    ...
    def remember_played(first_day: int) -> None:
        ...
        logging.info(
            "данные за модельный день %s отправлены; world_position=%s",
            first_day,
            first_day + 1,
        )
...
def world_live():
    """Автоматически запускать модельные дни, пока даг снят с паузы.

    Запускает `world_live_day` и ждет его завершения. Пауза останавливает
    запуск следующих дней; уже запущенный день завершится. Если он завершился
    с ошибкой, этот даг тоже завершится с ошибкой: откройте дочерний запуск
    и журнал неуспешной задачи.
    """
