"""Паспорт мира: чем стенд наполняется при подъёме и каким это обязано выйти.

Мир — чистая функция зерна (спека генератора, раздел 2), поэтому в git лежит не
он сам, а паспорт: шапка, число событий и хеш байтов каждого дня, счётчики
классов расхождений по заказам. Сам мир пересчитывается когда угодно, а паспорт
...
Дней в паспорте восемь: столько заливается в стенд при `make up`. Понедельник по
понедельник — полная неделя с выходными и первый замкнутый цикл окна K = 7.
Это и есть эталонный мир: обещание в четырнадцать дней снято на этапе 7 —
у хвоста не нашлось читателя. Понадобится длиннее — паспорт продолжит этот
же файл, а не заведет второй.
...
слепка нет — паспорт описывает мир, а не доставку.

Собирается паспорт из каталога генератора целью `make inventory`, а его свежесть
...
def build() -> dict[str, Any]:
    """Паспорт целиком: шапка, строка на день и строка на слепок.
...
def render() -> str:
    """Паспорт текстом файла: отступы в два пробела, кириллица как есть."""
    return json.dumps(build(), ensure_ascii=False, indent=2) + "\n"


def _day(number: int, today: day_module.Day) -> dict[str, Any]:
    """Строка паспорта: номер дня, его дата, число событий и хеш байтов.
...
def _snapshot(number: int, orders: list[orders_module.Orders]) -> dict[str, Any]:
    """Строка паспорта слепка: какой день снят, его дата и хеш отправленных байтов.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Собирает паспорт мира: шапку, счётчики и хеши дней."
    )
    parser.add_argument("output", type=Path, help="путь к файлу паспорта")
    output = parser.parse_args().output
    output.write_text(render(), encoding="utf-8")
    print(f"Паспорт мира собран: {output}")
