...
поэтому чек паспорта честно покраснеет: паспорт сторожит только канонический мир,
свои миры менти собирает без его гарантий (спека генератора, раздел 9).
...
# зависит — иначе паспорт перестал бы быть воспроизводимым.
...
# Событийная сторона сверки: потерянные маячки purchase и повторная
# отправка пары «просмотр подтверждения + purchase». Замер (28 дней,
# канонический мир, 2026-08-30): дубль 1,89% — в пределах ориентира ~2%;
# потеря 1,78% при броске 3% — так и задумано: назначенные планом покупки
# из броска исключены (мост склейки), часть съедает приоритет отмены;
# наблюдаемая доля принята решением владельца (#194).
LOST_PURCHASE_PERCENT = 3
DUPLICATE_PURCHASE_PERCENT = 2
DUPLICATE_DELAY_SECONDS = (30, 301)
...
# --- Числа заказов бэкенда: деньги магазина и судьба заказа ----------------
...
# заказной стороны. Замер (28 дней, канонический мир, 2026-08-30): 35,5% /
# 49,2% / 15,4% — совпадает с весами.
...
# Чем кончается окно изменяемости заказа: веса трёх исходов, в порядке
# `orders.OrderOutcome`. «Создан» навсегда мир не допускает — из окна заказ
# выходит либо оплаченным, либо отменённым. Замер (28 дней, 2026-08-30):
# отменено 5,15% заказов — 3,03% оплачен-потом-отменен, 2,12% отменен
# неоплаченным; ориентир мастер-спеки ~5%.
ORDER_OUTCOME_WEIGHTS = (
    95,  # оплачен
    3,  # оплачен, потом отменён
    2,  # отменен неоплаченным
)

# Когда внутри окна случается момент заказа — оплата или отмена: веса часов
# от 0 до 143, по часу от рождения заказа. Читается по парам «сколько часов —
# с каким весом»; часов в сумме ровно 144 — самый узкий край окна, у заказа,
# рождённого в конце суток (docs/architecture/orders/fate.md). Почти всё в
# первые сутки, дальше затухание до края:
# профиль гаснет, поэтому обрыв окна в данных не виден. Веса зафиксированы
# вместе с долями исходов — тем же замером выше.
ORDER_MOMENT_HOUR_WEIGHTS = tuple(
    weight
    for hours, weight in ((6, 100), (18, 60), (24, 25), (48, 8), (48, 2))
    for _ in range(hours)
)

# Доля заказов, у которых со склада вычеркнули позицию: товара не оказалось в
# наличии, и заказ приезжает на строку короче клиентской корзины. Бросается
# доля на все заказы дня, а достаётся дельта не всем — однопозиционному заказу
# терять нечего, и таких больше половины, — поэтому в сверке наблюдаемая доля
# выходит вдвое-втрое ниже броска, около ориентира мастер-спеки в 1–2%. Замер
# (28 дней, 2026-08-30): наблюдаемая доля 1,32% — в пределах ориентира.
ORDER_DELTA_PERCENT = 4

# На сколько первых дневных слепков заказ опоздает: веса δ = 0, 1, 2.
# Назначенные планом заказы после броска получают 0, чтобы мост склейки
# появился в стартовом мире. Замер (28 дней, 2026-08-30): доля опоздавших
# (δ>0) 8,39% — в пределах ориентира мастер-спеки ~10%.
ORDER_SNAPSHOT_DELAY_WEIGHTS = (90, 5, 5)
