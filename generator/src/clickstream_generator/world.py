...
# Часовой пояс счётчика — Самара, UTC+4 — записан двумя способами, потому что
# стороны просят разное. Генератору нужны минуты: в этом поясе считаются
# модельные сутки, как в выгрузке Метрики — `EventDate` дата в поясе счётчика,
# `UTCEventTime` абсолютная метка. Отсюда следствие, о котором сторона
# хранилища должна знать заранее: `toDate(UTCEventTime)` ≠ `EventDate` у ночных
# событий (спека генератора, раздел 9). Хранилищу тот же пояс нужен именем из
# базы поясов — так пишутся `toDate` и типы колонок DDS
# (docs/architecture/storage.md, «Часовые пояса»). Пути отсюда в SQL нет, имя
# переносят руками — но берут его здесь.
COUNTER_TIMEZONE = "Europe/Samara"
