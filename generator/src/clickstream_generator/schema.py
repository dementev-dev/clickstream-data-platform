...
- `name` — имя источника, как в облачной выгрузке Метрики; сырой слой хранит
  его без изменений.
- `clickhouse_type` — тип колонки у источника в той записи, которой пользуется
  ClickHouse: параметры входят в имя целиком, без сокращений
  (`LowCardinality(String)`, `Array(Float64)`). Обычно ODS берет его без
  изменений. Исключение одно: денежный `purchaseRevenue` приезжает как
  Float64, а на границе хранения разбирается прямо в Decimal. Поэтому поле
  описывает выгрузку, а локальный тип ODS назван в документации хранилища.
...
COLUMNS: tuple[Column, ...] = (
    ...
    Column(
        ...
        group=ColumnGroup.ECOMMERCE,
        comment="выручка заказа глазами клиента; источник объявляет Float64,"
        " ODS хранит точный Decimal",
    ),
    Column(
