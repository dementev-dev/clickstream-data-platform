...
# Окно замера плана — две недели: два цикла недельной сезонности.
WINDOW_DAYS = 14
...
def test_counters_are_a_pure_function_of_the_seed():
    first = plan.counters(CANONICAL_SEED, WINDOW_DAYS)
    plan.cohort.cache_clear()
    assert first == plan.counters(CANONICAL_SEED, WINDOW_DAYS)
...
def test_the_daily_audience_matches_the_spec_band():
    ...
    counters = plan.counters(CANONICAL_SEED, WINDOW_DAYS)
...
def test_the_influx_is_visible_on_the_plan():
    ...
    fortnight = plan.counters(CANONICAL_SEED, WINDOW_DAYS)
