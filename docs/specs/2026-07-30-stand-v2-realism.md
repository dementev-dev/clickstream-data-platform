...
- **`daily_traffic_v`** — расширяется парой «посетители» / «известные
  пользователи» (обогащение через `dds.identity_map_v`, локальное соединение
  по ключу ко-локации).
...
- Точную форму локального обогащения `daily_traffic_v` через публичные `_v` —
  эмпирически при проектировании витрин #135. Общее поведение соединения двух
  Distributed-таблиц уже проверено в
  [исследовании](../research/2026-08-22-distributed-join.md).
