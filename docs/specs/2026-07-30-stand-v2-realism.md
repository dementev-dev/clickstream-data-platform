...
- `ods.order` принимает версии заказа в
  ...
  слепка служит колонкой версии — выживает строка позднейшего слепка
  ([ADR 0014](../adr/0014-order-version-snapshot-date.md)), — но публикацию
  и идемпотентность по-прежнему не задаёт. Физическое чтение может видеть
  несколько версий; `ods.order_v` возвращает точное текущее состояние. В DDS одна строка хранит
...
и ошибки из того же среза. `ods.order` хранит версии по дате слепка
([ADR 0014](../adr/0014-order-version-snapshot-date.md)), а не публикует
партицию `snapshot_date`. Слой сырья остаётся точкой повтора и
