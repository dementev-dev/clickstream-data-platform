...
Потребительские роли и витрины читают сущности DDS через `_v` с явным списком
колонок; прав на `_rep` и `_dist` у них нет. Физические объекты остаются ролям
загрузки, диагностики и обслуживания. Механизм и проверяемая граница доступа —
в [доке хранилища](../architecture/storage.md), «Имена объектов».
