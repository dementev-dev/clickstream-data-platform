...
| B | Потерянное событие | заказ есть, `purchase` не доехал | бросок 3%, наблюдаемых ~1,8%: часть маячков защищена парами склейки, часть уходит в отмену приоритетом |
...
Пятое — **опоздание** — бесплатно даёт формат доставки: часть заказов впервые
появляется в слепке D+1/D+2 («вчера не сходилось, сегодня сошлось»), ориентир
~10%. Точные доли зафиксированы замером на каноническом мире (28 дней,
2026-08-30, #194) — запись в «Что проверено» [карты
проверок](../architecture/testing.md); паспорт хранит счётчики наблюдаемых
классов после приоритета — в заказах и только по дням с закрытым окном (раздел
8).
...
- **N:1**: часть покупателей покупает с двух кук («телефон и ноутбук») —
  параметр мира; значение фиксирует эта спека: 15%, детерминировано от seed.
  Ядро лабы: `uniq(посетителей) > uniq(людей)`, менти выводит расхождение сам.
  Константа мира: каждый двухкуковый покупатель делает минимум по одному заказу
  с каждой куки — иначе вторая кука не попадает в карту соответствий (она
  строится только из покупок) и лаба не воспроизводится. Число таких пар растёт,
  пока мир едет, и в паспорт не кладётся ([что хранит
  паспорт](../architecture/orders/inventory.md)).
...
- **Приём Kafka**: у событий — Kafka-таблицы и MV на обеих нодах, одна
  consumer group, 2 партиции на топик `hits`; MV пишут в Distributed-цели. У
  заказов приём другого рода — пакетный забор без MV, чтец на одной ноде, топик
  `orders` в одну партицию ([ADR 0008](../adr/0008-order-ingestion.md)).
  Раскладку в обоих случаях решает ключ: события — по `cityHash64(ClientID)`
  (см. 1.3), заказы — `cityHash64(order_id)`, STG — `cityHash64(сырой строки)`;
  полный список и доводы — в [доке хранилища](../architecture/storage.md). Урок
  «какая нода читала топик — меняется между прогонами, куда легли данные — нет»
  живёт на событиях: при пакетном заборе читает та нода, которую спросили.
...
суффиксов; она же задаёт служебные колонки, нарезку и срок хранения сырых данных
— см. [доку хранилища](../architecture/storage.md). Потребительские запросы и
витрины читают сущности DDS через `_v` с явным списком колонок; `_rep` и `_dist`
служат загрузке, диагностике и обслуживанию. Договор держится конвенцией, а не
правами: модель доступа описана в [ADR 0007](../adr/0007-clickhouse-access.md),
договор чтения — в [доке хранилища](../architecture/storage.md), «Имена
объектов».

Заказы принимаются **пакетным забором** ([ADR
0008](../adr/0008-order-ingestion.md), [ADR
0010](../adr/0010-order-versions-in-ods.md)): чтец топика байтовый, как у
событий, но матвью к нему не привязана, и порцию в STG забирает шаг под
управлением Airflow — одно прямое чтение вставляет её в `stg.orders_raw` с
`_load_id` запуска. Один следующий `task_id` двумя последовательными запросами
пишет годные строки и ошибки из того же среза. `ods.order` выбирает текущую
версию по максимальной `snapshot_date` ([ADR
0014](../adr/0014-order-version-snapshot-date.md)). Слой Сырой слой остаётся
точкой повтора и разбора одной принятой порции.
...
## 8. Эталонный мир и паспорт

В git хранится только паспорт эталонного мира; сам снимок генерируется на месте
— при `make up` и при проверках (решение развилки «Производительность» карты
#26, подробности — [спека генератора](2026-08-01-generator.md), раздел 5).
Снимок — 8 дней, совпадает со стартовым миром: обещание в 14 дней снято на этапе
7. Паспорт несёт шапку с каноническим зерном и версией генератора, контрольные
счётчики и хеши по дням; проверки «пустой git diff» и «пересгенерируй день N —
сравни хеш» живут на нём. Политика версионирования артефакта (бывший туман карты
#10) закрыта этим же ходом: версионируется паспорт. Контрольные числа паспорта:

- по строке на каждый отправленный слепок — хеш байтов: побайтовое обещание
  «слепок переснимается и даёт те же байты» паспорт сторожит наравне с днями;
- заказная сторона: счётчики наблюдаемых классов расхождений после
  ...
  закрытым окном (читатель — техническая проверка `classes_vs_inventory` звена
  DQ; при N сыгранных днях таких дней N − 7). Отдельных полей для общего числа
  заказов и выручки в паспорте пока нет: для них не назван читатель;
- контрольные числа идентичности не заводятся: uniq известных пользователей
  ...
  названного читателя в паспорт не кладётся ([что хранит
  паспорт](../architecture/orders/inventory.md)).
...
| Эталонный мир | пересборка снимка на месте, счётчики паспорта, чек-скрипты | M–L |
...
6. Расхождения B+D и опоздания; счётчики паспорта.
7. Эталонный мир: паспорт и пересборка снимка, чек-скрипты. CI-генерация
...
- Лабы и курс: v2 — другой стенд, лабы для него пишутся с нуля отдельной
  ...
  переносится. Исключение — домашки слоя DM: ТЗ витрин менти и даг самопроверки
  забраны в этап 4 резолюцией #135, потому что витрина менти и конвейер, в
  который она встает, проектируются вместе. Сюда же — необязательная лаба «свой
  дашборд в Superset» (#197): она стоит на витринах домашек DM и продумана до
  готового ТЗ в этапе 8, тогда как остальные лабы пока на уровне идей. Единый
  стиль блока домашек — за работой учебного пути. Спека даёт будущим лабам
  только опорные точки — контрольные числа паспорта (сверка, идентичность).
  Явное следствие: после этапа 9 стенд работает, но учебного пути на нём ещё
  нет.
