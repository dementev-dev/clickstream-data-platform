...
- **Корзина отвязана от страницы корзины.** Замерено до правки (день 2
  канонического мира): множества «визиты с событием корзины» и «визиты со
  страницей `/cart`» совпадали в точности — 802 и 802. Событие корзины
  ...
  (в том же дне 802 корзины против 272 покупок); правка добавляет к ним
...
- **Границы правки — те же вилки раздела 5**: аудитория 6–8 тыс., средний
  день около 50 тыс. событий (сейчас 49,5 тыс. — запас невелик, и рост
  событий корзины считается против него), конверсия визита около 2%.
  Заказ рисуется из корзины, а корзина сузилась, поэтому доля брошенных
  позиций (15%) и состав заказа (1,9 позиции) перемеряются вместе с
  остальным: сохраняется не число, а свойство — заказ уже корзины и
  однопозиционных заказов около половины. Числа канонического мира после
  правки перемеряются и переписываются.
