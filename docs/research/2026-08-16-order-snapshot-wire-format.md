...
используется как стабильный технический ключ партиции `ods.order`:
...
> Вывод о колонке версии пересмотрен: версию `ods.order` задаёт `snapshot_date`
> ([ADR 0014](../adr/0014-order-version-snapshot-date.md)). Там же разобрано,
> почему обещанию документации про ничью — «побеждает вставленная позже» —
> верить нельзя: замер на стенде его опроверг. Абзац выше оставлен как был,
> документ историчен.
