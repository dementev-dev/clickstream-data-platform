...
DDS. `snapshot_date` в нём — дата наблюдения и колонка версии, но не ключ
публикации. Полное решение — в
[ADR 0010](../adr/0010-order-versions-in-ods.md), колонка версии —
в [ADR 0014](../adr/0014-order-version-snapshot-date.md); механика приёма —
в [спецификации приёма заказов](orders/ingestion.md). Какой моделью заказ
живёт дальше — [модель заказа в DDS](dds/order.md).
