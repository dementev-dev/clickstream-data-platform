...
Для дневной пересборки DDS и DM используют донор — служебную таблицу,
в которой готовят результат перед заменой партиций. Например,
`dds.order_stage_rep` объявлена через `AS dds.order_rep`, а запись
в нее идет через `dds.order_stage_dist` с тем же `cityHash64(order_id)`.
У цели и донора должны совпадать структура, ключи партиции и сортировки,
первичный ключ и политика хранения. Эти требования сверены через Context7
с [документацией ClickHouse](https://clickhouse.com/docs/sql-reference/statements/alter/partition#replace-partition)
для используемой схемы на ClickHouse 26.3. Поэтому донор объявлен через
`AS` цели с совпадающими ключами. DDL показан в
[`sql/ddl/45-dds-tables.sql`](../../sql/ddl/45-dds-tables.sql).

`REPLACE PARTITION` копирует готовую партицию между локальными таблицами
на каждом шарде; заново распределять строки по ключу цели эта операция
не будет. Поэтому донор должен сразу иметь ту же раскладку, что и цель.
