...
повтор доставки. У `ods.order` версию задаёт `updated_at` источника;
...
[спецификации приёма заказов](orders/ingestion.md). Какой моделью заказ
...
через распределённую таблицу вернул одну строку — ту, у которой `updated_at`
позже, — тогда как физический счёт показывал две. Повтор задачи с тем же
