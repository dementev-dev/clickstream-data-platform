...
`Decimal(18, 2)`, `productPrice` события — целые рубли. Разрыв намеренный, на
нём стоит урок про `Float64` ([описание
...
| `55-dm-tables.sql` | физические витрины выручки и трафика, донор партиций выручки |
...
карта пишет полный набор пар в версионную таблицу. В DM выручка заменяет
партиции изменяемых дней, а трафик пишет полную историю новой версией.
