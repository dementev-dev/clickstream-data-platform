...
шарде, и шард пишет прямо в локальную таблицу цели, минуя её ключ шардирования.
Строки наследуют раскладку источника, и ошибки при этом нет: расхождение видно
только пересчётом по `hostName()` — сколько ключей встретилось больше чем на
одном шарде. На разборе заказов так и вышло: 1189 из 1662 заказов легли
одинаковыми строками на оба шарда, а `ReplacingMergeTree` через границу шарда их
не схлопывает. Поэтому у такой вставки в `SETTINGS` стоит явное
`parallel_distributed_insert_select = 0`: раскладку делает инициатор, по ключу
цели (`sql/ods/order_parse.sql`). Правило нужно и дальше — сборки DDS тоже будут
читать `_dist` и писать в `_dist` с другим ключом.
...
`_load_id` — ни одного, каждый шард держит только свои ключи. Вершина версий
уцелела: деловые поля `ods.order_v` сошлись с эталоном, посчитанным прямо из
сырья.
