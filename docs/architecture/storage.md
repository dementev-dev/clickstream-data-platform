...
и витрины называют только представление; `_rep` и `_dist` остаются ролям
загрузки, диагностики и обслуживания. Представление перечисляет колонки явно,
даже если его запрос — простая проекция над `_dist`: договор не выдаёт
устройство кластера и случайно добавленные физической таблице колонки. У
события физической копии в DDS нет, но правило то же: `dds.event_v` — его
публичный договор.

Граница доступа — часть договора, а не следствие имени. Обычное представление
создаётся с явными `DEFINER = CURRENT_USER SQL SECURITY DEFINER`: определивший
его пользователь читает источники, потребительская роль получает `SELECT` на
`_v`, физические объекты — роль загрузки или диагностики. При стандартном
`INVOKER` потребителю нужны были бы права и на источник ([документация
ClickHouse](https://clickhouse.com/docs/sql-reference/statements/create/view#sql-security)).
Значит реализация публичной поверхности закончена, когда запрос
потребительской ролью проходит через `_v` и отклоняется для `_rep` и `_dist`.
...
наполняет ODS. Второе: матвью приёма создаётся последней из всех, и потому
нарушает нумерацию слоёв. Kafka-движок начинает читать топик, когда к нему
