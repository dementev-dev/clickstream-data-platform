...
получил первую физическую модель слоя — заказ: его пересобирает заменой
партиций даг `dds_transform`.
...
| `45-dds-tables.sql` | физическая модель заказа и таблица-двойник, донор её партиций |
| `50-dds-views.sql` | публичные договоры события и заказа на языке модели DDS |
...
В слое DDS построены событие и заказ, и устроены они намеренно по-разному:
событие остаётся представлением над ODS, заказ получил физическую модель.
Сессии, карту идентичностей и слой DM принесут следующие тикеты этапа. Их
состав задан разделом 7 мастер-спеки и переносится сюда по мере постройки.
