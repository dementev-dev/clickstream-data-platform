...
**Сборка окна.** Слепок дня D берет заказы, рожденные в дни D−6…D, а несет из
них те, что уже попали в выгрузку: механика опоздания живет в
[судьбе заказа](fate.md). Окно собирается переигровкой этих семи дней: заказы
дня — производная всей воронки дня, дешёвого пути к ним нет. Цена — семь
проигрышей дня (~14 с) на слепок; у начала оси окно усекается само. Отклонено:
*кэш заказов на томе* — состояние между прогонами; *окно держит хранилище* —
топик перестаёт нести слепок; *K = 1* — это страховочный срез 1 мастер-спеки,
он в резерве.
...
порядок рождения заказов, он же возрастание `order_id`**. Детерминизм даёт его
даром, а хешу слепка в паспорте нужен именно названный порядок.
