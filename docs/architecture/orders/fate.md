...
а с d+δ она появляется в том состоянии, до которого заказ дожил: отменённый
на второй день и опоздавший на третий приедет в первом же своём слепке как
`cancelled` — «выгрузка догоняет жизнь». Таблица задержки состоит из трех
строк: δ = 0/1/2 с весами 90/5/5. 1 и 2 - это D+1 и D+2 мастер-спеки. Бросок
идет последним в подпотоке `ORDERS`, затем назначенному планом заказу δ
обнуляется. Меряется задержка в днях снятия слепка, а не отправки: иначе
сдвиг отправки удвоился бы, и обещанные D+1/D+2 стали бы D+2/D+3. Дальше
таблица не идет: заказ с δ = 6 приехал бы ровно в одном слепке, и
обещание «пропущенный день ничего не ломает» на нем перестало бы быть
верным; при δ ≤ 2 у всякого заказа слепков не меньше пяти.
