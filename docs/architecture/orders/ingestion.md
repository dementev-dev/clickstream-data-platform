...
неизменного `created_at`. `ods.order_dist` шардирует по
...
Версию выбирает только `updated_at`. Заказ, не менявшийся несколько дней,
приезжает в нескольких слепках одинаковыми деловыми полями, и какая из этих
строк переживёт схлопывание — не обещано. Поэтому `snapshot_date` выжившей
строки — один из дней, когда источник показывал эту версию, не обязательно
последний; то же с `_load_id` и `_load_ts`. Вопрос «когда заказ наблюдался в
последний раз» задаётся сырью STG, пока то живо, — а не одной строке ODS.

Сама схема версий стоит на допущении об источнике: уточнение заказа поднимает
`updated_at`. Генератор моделирует источник с триггерной меткой и допущение
выполняет (замер — «Что проверено»); источник, умеющий уточнить строку тихо —
ручным `UPDATE` мимо приложения, миграцией, — ломает его, и тогда версии по
метке времени мало: нужна сверка самих полей, в терминах dbt snapshots —
стратегия `check` вместо `timestamp`.
...
Допущение и произвол ярлыков замерены 22 августа 2026 года при исполнении
#137: во всех 3034 парах строк с равным `updated_at` деловые поля совпали
побайтово, по слепкам метка ни разу не убывала, а `ods.order_v` сошёлся
построчно с эталоном `argMax` по `updated_at`, посчитанным прямо из сырья.
Произвол ярлыков виден въявь: у 86 заказов стартового мира выжила строка
серединного слепка — не первого и не последнего.
