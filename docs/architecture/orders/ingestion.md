...
Одно условие контракта предикатом не проверить, и названо оно всё равно
здесь: **одна дата — один слепок**. Вторую выгрузку того же дня приём не
отличит от первой, а колонка версии — дата слепка — на ней даёт ничью.
Модельный источник выгружает день один раз; настоящему такое условие ставят
договором, а не предикатом
...
Версию выбирает `snapshot_date`: выживает строка позднейшего слепка
([ADR 0014](../../adr/0014-order-version-snapshot-date.md)). Поэтому ярлык
выжившей строки — правдивый ответ на вопрос «когда заказ видели в последний
раз», и по нему трансформация DDS читает объём пересборки. `_load_id` и
`_load_ts` выжившей строки говорят про ту же доставку.
...
Дисциплина источника замерена 22 августа 2026 года при исполнении #137: во
всех 3034 парах строк с равным `updated_at` деловые поля совпали побайтово, по
слепкам метка ни разу не убывала, а `ods.order_v` сошёлся построчно с эталоном
`argMax` по `updated_at`, посчитанным прямо из сырья. Тем же замером виден
произвол ярлыка при версии по деловой метке — довод, на котором стоит
[ADR 0014](../../adr/0014-order-version-snapshot-date.md).

Правдивость ярлыка снята на пересобранном мире 26 августа 2026 года при
исполнении #165. После двух слепков `snapshot_date` каждой из 1917 строк
`ods.order_v` равен последнему слепку, который нёс её заказ: расхождений ноль
при 3356 физически лежащих строках — выбор был не единственным. Деловое
содержимое сменой колонки версии не тронулось: те же строки построчно совпали
с эталоном `argMax` по `updated_at` из сырья по всем полям.
