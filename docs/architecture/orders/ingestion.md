...
У выжившей после схлопывания строки три последние координаты означают меньше,
чем кажется. Версию выбирает только `updated_at`; заказ, не менявшийся
несколько дней, приезжает в нескольких слепках одинаковыми деловыми полями, и
какая из этих строк переживёт схлопывание — не обещано. Поэтому её
`snapshot_date` — *один из* дней, когда источник показывал эту версию, не
обязательно последний; то же с `_load_id` и `_load_ts`. Замер 22 августа
2026 года (#137): у 86 заказов стартового мира выжил даже не крайний день, а
серединный. Вопрос «когда заказ наблюдался в последний раз» задаётся сырью STG,
пока его не съел срок жизни, — а не одной строке ODS.

Вся схема версий стоит на одном свойстве источника: уточнение заказа поднимает
`updated_at`. Наш генератор моделирует источник с триггерной меткой и это
свойство гарантирует; замеры 22 августа 2026 года его подтвердили — во всех
парах строк с равным `updated_at` деловые поля совпали побайтово, а по слепкам
метка не убывала ни разу. Это допущение, а не закон природы: источник, умеющий
уточнить строку тихо — правка мимо приложения, миграция, ручной `UPDATE`, —
эту схему ломает, и тогда версии по метке времени уже не хватает, нужна сверка
самих полей (в терминах dbt snapshots — стратегия `check` вместо `timestamp`).
