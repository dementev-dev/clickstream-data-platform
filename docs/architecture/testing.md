...
**Приёмка #119 21 августа 2026 года.** `make rebuild-storage` через Airflow
прошёл за 3 м 9 с: позиция прошла задачи `-1` → `0` → `8`, топики получили
новые идентификаторы, группы дочитали новые топики без отставания, а ODS принял
401 185 событий без брака и 2 851 актуальный заказ. Повторный `make up` занял
2 м 41 с вместе со штатным повторным запуском служб инициализации;
`world_initialize` завершился за один цикл опроса и оставил счётчики
неизменными. Отдельно
проверены красные отказы `world_initialize` при `-1` и `world_next_day` при
отсутствующей позиции, `0` и `-1`; `lifecycle` отказано в чтении
`system.users`, но разрешено чтение `system.settings`.
...
`orders` один, отставание обеих групп равно нулю. На этом стенде прошли смоук
20/20, проверка ClickHouse 10/10 и межсервисная проверка 7/7.
