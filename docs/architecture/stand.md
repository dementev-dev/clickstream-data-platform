...
остаются в [корневом README](../../README.md#адреса-и-пароли).
...
В ClickHouse восемь пользователей и семь ролей:
...
- `analyst` — учётная запись аналитика с правом чтения всех слоёв и
  справочников;
- `student` — учебная учётная запись: роль `analyst_reader` дает чтение
  источников, а `student_writer` — создание, заполнение и удаление своих
  таблиц в `default`. Записи в слои хранилища у нее нет;
...
Для существующего стенда сначала добавьте `CLICKHOUSE_STUDENT_PASSWORD`
из `.env.example` в свой `.env`.
