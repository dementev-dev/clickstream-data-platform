...
21 августа 2026 года дашборд «Данные» собран. Один пункт решения исполнен не
буквально: у панели потока нет ряда «против вчерашнего». Сдвинуть ряд на
...
Тогда же собраны «Кластер» и «Запросы». Довод про `clusterAllReplicas`
подтвердился с одной поправкой: очередь распределённых DDL лежит не у ноды, а в
keeper, и обе ноды читают её целиком — запрос вернул с каждой одни и те же 124
записи. Обёртка там не добавила бы правды, а удвоила бы каждую строку, поэтому
единственная панель «Кластера» без неё — эта. Keeper своих системных таблиц не
имеет вовсе, и панель по нему приходит из Prometheus.

«Запросы» получили на две панели больше, чем сказано выше. Разбор запросов по
посетителям продиктован [ADR 0007](0007-clickhouse-access.md): своя учётка
Grafana заведена там ровно затем, чтобы её запросы были отличимы от
человеческих.
Отдельная панель ошибок понадобилась потому, что отказ достаётся шарду: в
замере с принудительным пределом памяти каждая нода записала свой, и строка
шарда видна только без фильтра по инициатору. Здешний сюжет про тяжёлый
`GROUP BY` при этом кончается не отказом, а сбросом на диск
([ADR 0004](0004-resource-limits.md)) — отказ на стенде приходится вызывать
настройкой. Устройство обоих дашбордов —
[cluster.md](../architecture/monitoring/cluster.md) и
[queries.md](../architecture/monitoring/queries.md).
