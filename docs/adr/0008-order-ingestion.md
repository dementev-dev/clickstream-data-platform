...
Позднее тикет #214 сменил имена: «сырьё» стало сырыми данными, «работник» —
шаговым дагом. Текст ниже оставлен в словах своего времени, действующие
имена — в `CONTEXT.md`.
...
меняется. `etl_pipeline` остаётся за этапом 5. Позднее сдвинулся и он:
резолюция #134 родила его этапом 4 — [дневным конвейером
трансформаций](../architecture/etl.md), — а этапу 5 оставила партиционную
переобработку.
