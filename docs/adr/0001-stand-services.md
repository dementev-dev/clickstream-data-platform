...
Каждый общий локальный образ собирает одна служба: `airflow-init` владеет
сборкой образа Airflow, `superset-init` — образа Superset. Зависимые процессы
только используют готовый образ. `docker compose up --build` собирает образы
до запуска контейнеров; повторять `build` у каждого процесса не нужно, а
Buildx 0.30.1 при параллельном экспорте одного тега иногда отвечает
