...
# в `ods.event` против паспорта мира, лежащего в git. Поэтому она умеет краснеть на
...
# этап 5 добавит следующий, — и голый счёт по таблице разойдётся с паспортом
...
check_starting_world() {
    ...
    if [[ "$actual" == "$expected" ]]; then
        printf 'ЗЕЛЁНО: события всех дней стартового мира на месте, счёт сходится с паспортом.\n'
        return
    fi
    ...
    broken="$(query clickhouse-01 "
        ...
        ORDER BY count() DESC
        FORMAT TSV")"
    printf 'Паспорт мира ожидает (дата, событий):\n%s\n' "$expected" >&2
    printf 'В ods.event лежит:\n%s\n' "${actual:-— ничего —}" >&2
    if [[ -z "$broken" ]]; then
        fail 'счёт разошёлся с паспортом, а таблица брака пуста: события не доехали до ODS — начните с чтеца топика stg.hits_raw_kafka и матвью приёма stg.hits_raw_mv'
    fi
    printf 'В ods.event_errors по классам брака:\n%s\n' "$broken" >&2
    fail 'счёт разошёлся с паспортом, и в таблице брака есть строки. Сойдётся недостача с их числом — сломан разбор, смотрите матвью ods.event_mv; не сойдётся — брак остался от прежних опытов, а события не доехали: смотрите чтеца топика stg.hits_raw_kafka'
}
...
printf 'Проверка 10/10: стартовый мир в ods.event сходится с паспортом...\n'
check_starting_world
